...
import utils.db.redis.utils.BeanUtil;
...
public class RedisUtil {
	...
	private volatile static RedisUtil singleton = null;
	...
	private RedisUtil() {
		if (LogUtil.getInstance().getLogger(RedisUtil.class).isDebugEnabled()) {
			LogUtil.getInstance().getLogger(RedisUtil.class).debug("MongodbCollectionManagerUtil() - start "); //$NON-NLS-1$
		}
		...
		if (mg==null) {
			try {
				...
				if (DataTypeUtil.isNotEmpty(InitRedis.SERVER_HOSTS)) {
					isShard = true;
					LogUtil.getInstance().getLogger(this.getClass()).info("-----启动集群分片数据库：{}",InitRedis.SERVER_HOSTS);
					String[] list = InitRedis.SERVER_HOSTS.split(",");
					for (String ip : list) {
						ServerAddress address = new ServerAddress(ip, InitRedis.SERVER_PORT);
						addresses.add(address);
					}
					mg = new MongoClient(addresses);
				}else{
					isShard = false;
					LogUtil.getInstance().getLogger(this.getClass()).info("启动单机数据库：{}",InitRedis.SERVER_HOST);
					mg = new MongoClient(InitRedis.SERVER_HOST, InitRedis.SERVER_PORT);
				}
				dbs.put(InitRedis.DATABASE_INFO, getDB(InitRedis.DATABASE_INFO));
				dbs.put(InitRedis.DATABASE_MAKE, getDB(InitRedis.DATABASE_MAKE));
				dbs.put(InitRedis.DATABASE_TRACK, getDB(InitRedis.DATABASE_TRACK));
		...
		if (LogUtil.getInstance().getLogger(RedisUtil.class).isDebugEnabled()) {
			LogUtil.getInstance().getLogger(RedisUtil.class).debug("MongodbCollectionManagerUtil() - end"); //$NON-NLS-1$
		}
	}
    ...
    public boolean remove(String databasename,String collectionname, HashMap<String, Object> map) { 
        ...
        if (null != writeResult) {
            result = getWriteResult(result, writeResult);
            if (!result) {
            	LogUtil.getInstance().getLogger(RedisUtil.class)
     ...
     * @return
     */
    public <T> List<T> getByMultiCondition(String databasename,String collectionname, RedisCondition condition,
